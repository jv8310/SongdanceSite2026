-- Seed the "Songdance Story · Part 4 — The Birth of Somatic Sounding" broadcast
-- as a DRAFT — the final chapter of the origin-story arc (0051 ASJ, 0060 MMJ,
-- 0061 Grief). Site-list twin of docs/emails/svh-birth-story.drip.html: same
-- body, but utm_source=broadcast, the single one-click {{ unsubscribe_url }}
-- footer, and the literal postal line. Blank audience = the whole sendable list.
-- Appears in /admin/broadcasts ready to review, test-send, and launch; nothing
-- sends until launched. Idempotent via the NOT EXISTS name guard.

INSERT INTO broadcasts (name, subject, preheader, heading, body, format, status)
SELECT 'Songdance Story · Part 4 — The Birth of Somatic Sounding',
       'The birth of somatic sounding',
       'How the sounding began — the last chapter of the Songdance story.',
       'The birth of somatic sounding',
       '<!DOCTYPE html>
<html lang="en">
<head>
...
<tr><td class="pad" style="padding: 12px 44px 6px;">
  <p style="font-family: ''Figtree'', -apple-system, BlinkMacSystemFont, ''Segoe UI'', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 1.7; color: #2A1B2A; margin: 0 0 16px;">That is what somatic sounding is: making the sound of what&rsquo;s stored in the body. Sounded, the stuck energy is finally met &mdash; and life can be lived from a new whole, restructured by your own acknowledgment.</p>
  ...
  <p style="font-family: ''Figtree'', -apple-system, BlinkMacSystemFont, ''Segoe UI'', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 1.7; color: #4A3848; margin: 0;">Even the ego belongs in this story &mdash; the ego is nothing more than consolidated bodily memory in action. You don&rsquo;t transcend it; you help it grow up, by integrating what was into a new whole. The bigger it grows, the better &mdash; until one day it&rsquo;s big enough to realize, from lived, embodied experience, that it is the whole universe. And then, only then, it transcends itself. I&rsquo;m drifting far now &mdash; but that&rsquo;s exactly the point. Somatic Vocal Healing touches all of these layers. It isn&rsquo;t only about healing. It&rsquo;s about growing up into a more conscious human being.</p>
</td></tr>
...
',
       'html', 'draft'
WHERE NOT EXISTS (SELECT 1 FROM broadcasts WHERE name = 'Songdance Story · Part 4 — The Birth of Somatic Sounding');
