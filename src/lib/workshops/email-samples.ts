...
export function buildEmailSamples(base: string): EmailSample[] {
  ...
  const courseUrl = `${b}/courses/12-week?email=maria%40example.com`;
  ...
  return [
    ...
    // ── Song Deck gift confirmed — order placed on Shopify (transactional) ──
    {
      ...
      group: 'Registration & reminders (transactional)',
      label: 'Songdeck gift — order placed, shipping soon',
    ...
    },
    {
      id: 'date_changed',
      group: 'Registration & reminders (transactional)',
      label: 'Date change — moved to a new date',
      timing: 'When someone switches dates from their countdown page',
      audience: 'Registrants who move their seat to another live date',
      content: dateChangedEmail({
        name,
        workshopTitle,
        whenLocal,
        joinUrl,
    ...
    },
    {
      id: 'reminder_20m',
      group: 'Registration & reminders (transactional)',
      label: 'Reminder — 20 minutes (lean; just the door — also 6h/1h)',
      timing: '20 min before the session',
      audience: 'Everyone registered',
      content: reminderEmail('reminder_20m', { name, workshopTitle, whenLocal, joinUrl }),
    },
    {
      id: 'reminder_5m',
      group: 'Registration & reminders (transactional)',
      label: "Reminder — we're live (the room is open; catches up if a tick is missed)",
      timing: '5 min before the session',
      audience: 'Everyone registered',
      content: reminderEmail('reminder_5m', { name, workshopTitle, whenLocal, joinUrl }),
    },
    {
      id: 'verification',
      group: 'Registration & reminders (transactional)',
    ...
    // ── Internal reports (SD-REPORT, ops only) ───────────────────────────
    {
      ...
      audience: 'Team inbox (REPORTS_TO) — yesterday vs the same weekday last week: sold (payment plans in full) vs cash in, ad economics per product, registrations, the next sessions, course sales, bumps',
    ...
    },
    {
      ...
      audience: 'Team inbox (REPORTS_TO) — the week vs the week before: same sections + the open-plan pipeline and a by-day table (regs, ad spend, sold, cash in)',
